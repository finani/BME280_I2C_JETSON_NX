...
BME280::BME280(void)
  : modeVector({"SLEEP", "FORCED", "FORCED", "NORMNAL"}),
  I2C_ADDR(0x77),
  tStandby(0),
  filter(0),
  tempOverSample(1),
  ...
  referencePressure(101325.0)
{
  ...
  if (fd < 0) {
    std::cout << "[BME280::BME280], Error opening file: " << strerror(errno) << std::endl;
  }
  if (ioctl(fd, I2C_SLAVE, I2C_ADDR) < 0) {
    std::cout << "[BME280::BME280], ioctl error: " << strerror(errno) << std::endl;
  }

  readCalibrationData();

  setStandbyTime(tStandby); // [0] 0.5ms, [1] 62.5ms, [2] 125ms, [3] 250ms, [4] 500ms, [5] 1000ms, [6] 10ms, [7] 20ms
  setFilter(filter); // [0] filter off, [1] coefficients = 2, [2] coefficients = 4, [3] coefficients = 8, [4] coefficients = 16
  setPressureOverSample(pressOverSample); // [1] Default of 1x oversample
  setHumidityOverSample(humidOverSample); // [1] Default of 1x oversample
  setTempOverSample(tempOverSample);      // [1] Default of 1x oversample

  
  uint8_t mode = getMode();
  if (mode != MODE_NORMAL) {
    setMode(MODE_NORMAL); // [b00] Sleep, [b01 or b10] Forced, [b11] Normal mode
  }
  std::cout << "[BME280::BME280], mode: " << modeVector[getMode()] << std::endl;
  std::cout << std::endl;
}
...
float BME280::getReferencePressure() {  return referencePressure; }
float BME280::getTempC(void) {  return tempC; }
float BME280::getTempF(void) {  return tempF; }
float BME280::getPressure(void) {  return pressure; }
float BME280::getAltMeters(void) {  return altMeters; }
float BME280::getAltFeet(void) {  return altFeet; }
float BME280::getHumidity(void) {  return humidity; }
float BME280::getDewpointC(void) {  return dewPointC; }
float BME280::getDewpointF(void) {  return dewPointF; }

// [b00] Sleep, [b01 or b10] Forced, [b11] Normal mode
void BME280::setMode(uint8_t mode_) {
  if(mode_ > 0b11) mode_ = 0; //Error check. Default to sleep mode

  uint8_t controlData = readRegister(BME280_REG::BME280_CTRL_MEAS_REG);
  controlData &= ~( (1<<1) | (1<<0) ); //Clear the mode[1:0] bits
  controlData |= mode_; //Set
...
void BME280::setReferencePressure(float refPressure_)
{
  referencePressure = refPressure_;
}

// [0] 0.5ms, [1] 62.5ms, [2] 125ms, [3] 250ms, [4] 500ms, [5] 1000ms, [6] 10ms, [7] 20ms
void BME280::setStandbyTime(uint8_t timeSetting_) {
  if(timeSetting_ > 0b111) timeSetting_ = 0; // Error check. Default to 0.5ms

  uint8_t controlData = readRegister(BME280_REG::BME280_CONFIG_REG);
  controlData &= ~( (1<<7) | (1<<6) | (1<<5) ); //Clear the 7/6/5 bits
  controlData |= (timeSetting_ << 5); // Align with bits 7/6/5
...
void BME280::setFilter(uint8_t filterSetting_) {
  if(filterSetting_ > 0b111) filterSetting_ = 0; // Error check. Default to filter off

  uint8_t controlData = readRegister(BME280_REG::BME280_CONFIG_REG);
  controlData &= ~( (1<<4) | (1<<3) | (1<<2) ); // Clear the 4/3/2 bits
  controlData |= (filterSetting_ << 2); // Align with bits 4/3/2
  writeRegister(BME280_REG::BME280_CONFIG_REG, controlData);
}

// [0] turns off temp sensing, [1-16] are valid over sampling values
void BME280::setTempOverSample(uint8_t overSampleAmount_) {
  overSampleAmount_ = checkSampleValue(overSampleAmount_); // Error check
  ...
  controlData |= overSampleAmount_ << 5; // Align overSampleAmount to bits 7/6/5
  writeRegister(BME280_REG::BME280_CTRL_MEAS_REG, controlData);

  setMode(originalMode); // Return to the original user's choice
}

// [0] turns off pressure sensing, [1-16] are valid over sampling values
void BME280::setPressureOverSample(uint8_t overSampleAmount_) {
  overSampleAmount_ = checkSampleValue(overSampleAmount_); // Error check
  ...
  controlData |= overSampleAmount_ << 2; // Align overSampleAmount to bits 4/3/2
  writeRegister(BME280_REG::BME280_CTRL_MEAS_REG, controlData);

  setMode(originalMode); // Return to the original user's choice
}

// [0] turns off humidity sensing, [1-16] are valid over sampling values
void BME280::setHumidityOverSample(uint8_t overSampleAmount_) {
  overSampleAmount_ = checkSampleValue(overSampleAmount_); //Error check
  ...
  controlData |= overSampleAmount_ << 0; //Align overSampleAmount to bits 2/1/0
...
void BME280::writeRegister(uint8_t reg_addr_, uint8_t dataToWrite_) {
  uint8_t buffer[2] = {reg_addr_, dataToWrite_};
  write(fd, buffer, 2);
}
...
void BME280::readAllMeasurements(void) {
  ...
  dewPointC = calDewPointC();
  dewPointF = calDewPointF();
}
...
float BME280::readFloatAltitudeMeters(float pressure_) {
  return ((float)-44330.77)*(pow((pressure_/(float)referencePressure), 0.190263) - (float)1);
}

float BME280::readFloatAltitudeFeet(float pressure_) {
  return readFloatAltitudeMeters(pressure_) * 3.28084;
}
...
float BME280::calDewPointC(void) {
...
float BME280::calDewPointF(void) {
  return calDewPointC()*1.8 + 32; // Convert C to F
}

uint8_t BME280::checkSampleValue(uint8_t userValue) {
  switch(userValue) 
  {
    case(0):  return 0; break; // Valid
    case(1):  return 1; break; // Valid
    case(2):  return 2; break; // Valid
    case(4):  return 3; break; // Valid
    case(8):  return 4; break; // Valid
    case(16): return 5; break; // Valid
    default:  return 1; break; // Default to 1x
  }
}
...
void BME280::readTempFFromBurst(uint8_t buffer[]) {
  tempF = (readTempFromBurst(buffer) * 9) / 5 + 32;
}
